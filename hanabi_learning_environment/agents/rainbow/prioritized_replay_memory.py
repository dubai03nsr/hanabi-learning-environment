...
class OutOfGraphPrioritizedReplayMemory(replay_memory.OutOfGraphReplayMemory):
  ...
  def add(self, observation, action, reward, terminal, legal_actions, self_hand, tom1_hand):
    ...
    if self.is_empty() or self.terminals[self.cursor() - 1] == 1:
      ...
      for _ in range(self._stack_size - 1):
        self._add(dummy_observation, 0, 0, 0, dummy_legal_actions, dummy_self_hand, dummy_tom1_hand, priority=0.0)
  ...
  def _add(self, observation, action, reward, terminal, legal_actions, self_hand, tom1_hand,
    ...
    super(OutOfGraphPrioritizedReplayMemory, self)._add(
        observation, action, reward, terminal, legal_actions, self_hand, tom1_hand)
